...
<nav class="p-2.5 bg-white shadow-md md:flex md:items-center md:justify-between fixed top-0 w-full z-10 ">
  ...
  <div class="flex justify-center items-center">
      <div class="w-3/4">
        ...
        <ol class="pointer-events-none stepper flex items-center p-3 space-x-2 text-sm font-medium text-center text-gray-500 shadow-sm dark:text-gray-400 sm:text-base dark:bg-gray-800 dark:border-gray-700 sm:p-4 sm:space-x-4 rtl:space-x-reverse">
          ...
          <li class="flex items-center  text-blue-600 dark:text-blue-500">
            <span class="flex items-center justify-center w-5 h-5 me-2 text-xs border border-blue-600 rounded-full shrink-0 dark:border-blue-600">
              3
            </span>
            Finish !
          </li>
  ...
  <div>
    <span class="font-medium">Almost finish !</span> Please complete your payment .
  </div>
</div>
        <hr class="css-5t036d">
    ...
    paypal.Buttons({
      createOrder: function(data, actions) {
        return actions.order.create({
          purchase_units: [{
            amount: {
              value: '<?php echo $_SESSION['totalUSD'] ?>' // Replace with your order total
            }
          }]
        });
      },
        ...
        return actions.order.capture().then(function(details) {
          ...
          fetch('confirm_payment.php', {
            ...
            if (data.success) {
              ...
              window.location.href = 'success_order.php'; // Redirect to confirmation page
            } else {
